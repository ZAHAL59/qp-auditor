...
export default function App() {
  ...
  const handleStart = async ({ text, file }) => {
    ...
    try {
      ...
      if (!content || content.trim().length < 10) throw new Error('Could not extract text. Please paste the text instead.');
      const auditResult = await auditPaper(content);
      const numberedIssues = (auditResult.issues || []).map((issue, i) => ({ ...issue, id: i + 1, _state: 'pending' }));
  ...
  const handleReset = () => { setStep(STEP.UPLOAD); setResult(null); setIssues([]); setError(''); };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f8fa' }}>
      {step !== STEP.LOADING && (
        <div style={styles.stepBar}>
          {['Upload', 'Analyze', 'Review'].map((label, i) => {
            const num = i + 1;
            const cur = step === STEP.UPLOAD ? 1 : step === STEP.LOADING ? 2 : 3;
            const done = num < cur; const active = num === cur;
            return (
              <React.Fragment key={label}>
                <div style={styles.stepItem}>
                  <div style={{ ...styles.stepDot, background: done || active ? '#4f6ef7' : '#e2e5ea', color: done || active ? '#fff' : '#9099a8', opacity: active ? 1 : done ? 0.7 : 0.5 }}>{done ? '✓' : num}</div>
      ...
      {error && <div style={styles.errorBanner}>⚠️ {error}<button style={styles.errClose} onClick={() => setError('')}>×</button></div>}
      {step === STEP.UPLOAD && <UploadStep onStart={handleStart} loading={false} />}
      {step === STEP.LOADING && <LoadingStep />}
      {step === STEP.RESULTS && result && <ResultsStep result={result} issues={issues} setIssues={setIssues} onReset={handleReset} />}
      <footer style={styles.footer}>QP Auditor · Powered by Gemini AI</footer>
    </div>
  );
}
...
const styles = {
  ...
  stepDot: { width: 26, height: 26, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 500 },
  stepLabel: { fontSize: 11 },
  stepLine: { flex: 1, height: 1.5, margin: '0 4px', marginBottom: 18 },
